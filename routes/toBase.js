...
module.exports.postPost = function(req, res, next) {
    ...
    Post.publish(title, post, inTags, function(err, post) {
        ...
        for (i = 0; i < inTags.length; i++) Tag.updateTag(inTags[i], post, function(err) {
            if (err) console.log('Error in tag:' + err + inTags[i])
        });
        res.sendStatus(200);
    });
...
module.exports.updatePost = function(req, res, next) {
    ...
    Post.findById(req.params.page, function(err, page) {
        if (err) {
            console.log('loger' + err.message);
            return next(err);
        }
        ...
        page.save(function(err) {
            ...
            for (i = 0; i < inTags.length; i++) Tag.updateTag(inTags[i], post._id, function(err) {
                if (err) console.log('Error in tag:' + err + inTags[i])
            });

...
Tags.find({},function(err,tags){if(err) console.log(err); 
res.render('tags',{tags:tags});
	})
	}
	module.exports.users=function(req,res,next) {
Users.find({},function(err,users){if(err) console.log(err); 
/*
...
{
	Comments.remove({author:'local_'+req.params.id},function(err){if(err) console.log(err)});
	
}
else if(req.params.type=='savecom')
{
	Comments.find({author:'local_'+req.params.id}).cursor()
  .on('data', function(e){
...
Users.remove({_id:req.params.id},function(err){if(err) console.log(err); res.redirect('/users');})
	}
	module.exports.userComments=function(req,res,next) {
Comments.find({author:req.body.id},function(err,comments)
{
...
Posts.update({},{$pull:{tags:tag.tag}},{multi: true},function(err,result){if(err) console.log(err); 
res.redirect('/tags');
	})
Tags.remove({ _id: req.params.tag }, function(err) {if(err) console.log(err)})
	})
}
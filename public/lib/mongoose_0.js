...
Likes.statics.click=function(post_id,user_id,callback)
{
    var Likes=this;

    async.waterfall([//user_id не передается
        function(callback){
            Likes.findOne({user_id: user_id,post_id:post_id},callback);
        },
        function(like,  callback){
            if (like) {
Likes.remove({user_id: user_id,post_id:post_id},callback);
}
else
{
 var like = new Likes({post_id: post_id, user_id: user_id});
...
Likes.count({post_id:post_id},callback)
}            
